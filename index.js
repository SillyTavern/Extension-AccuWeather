...
function parseOpenMeteoForecastData(data, units) {
...
async function getWeatherByProvider(location, args) {
    ...
    switch (provider) {
        ...
        case 'open-meteo': {
            const data = await getOpenMeteoCurrentWeather(currentLocation, units);
...
async function getForecastByProvider(location, units) {
    ...
    switch (provider) {
        ...
        case 'open-meteo': {
            const data = await getOpenMeteoForecast(currentLocation, currentUnits);
            return parseOpenMeteoForecastData(data, currentUnits);
        }
        case 'accuweather':
        default: {
            if (!extension_settings.accuweather.apiKey) {
