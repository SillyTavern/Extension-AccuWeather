...
async function getOWMWeather(location, units) {
...
function parseOWMWeatherData(data, args) {
    ...
    if (!isFalseBoolean(args.condition)) {
        parts.push(current.weather[0].description);
    }
    if (!isFalseBoolean(args.temperature)) {
        let temp = `${Math.round(current.temp)}°${tempUnit}`;
        if (isTrueBoolean(args.feelslike)) {
            temp += ` (feels like ${Math.round(current.feels_like)}°${tempUnit})`;
        }
        parts.push(temp);
    }
    if (isTrueBoolean(args.wind)) {
        parts.push(`Wind: ${current.wind_speed} ${speedUnit} ${degreesToDirection(current.wind_deg)}`);
    }
    if (isTrueBoolean(args.humidity)) {
        parts.push(`Humidity: ${current.humidity}%`);
    }
    if (isTrueBoolean(args.pressure)) {
        parts.push(`Pressure: ${current.pressure} hPa`);
    }
    if (isTrueBoolean(args.visibility)) {
        const vis = isImperial ? `${(current.visibility / 1609.34).toFixed(1)} miles` : `${(current.visibility / 1000).toFixed(1)} km`;
        parts.push(`Visibility: ${vis}`);
    }
    if (isTrueBoolean(args.uvindex)) {
        parts.push(`UV Index: ${current.uvi}`);
    }
    if (isTrueBoolean(args.precipitation)) {
...
function parseOWMForecastData(data, units) {
    ...
    if (!daily || daily.length === 0) throw new Error('No forecast data available');
    const isImperial = String(units).trim().toLowerCase() === 'imperial';
    const tempUnit = isImperial ? 'F' : 'C';
    const start = new Date(daily[0].dt * 1000);
    const end = new Date(daily[Math.min(daily.length - 1, 4)].dt * 1000);
    const parts = [];
    parts.push(`Weather forecast for ${start.toLocaleDateString()}-${end.toLocaleDateString()}:`);
    for (let i = 0; i < Math.min(daily.length, 5); i++) {
        ...
        parts.push(`${dayDate.toLocaleDateString()}: ${description}. Temperature: ${Math.round(day.temp.min)}°${tempUnit} - ${Math.round(day.temp.max)}°${tempUnit}`);
    }
    return parts.join('\n');
}
...
async function getWeatherByProvider(location, args) {
    ...
    switch (provider) {
        ...
        case 'openweathermap': {
            ...
            const data = await getOWMWeather(currentLocation, units);
...
async function getForecastByProvider(location, units) {
    ...
    switch (provider) {
        ...
        case 'openweathermap': {
            ...
            const data = await getOWMWeather(currentLocation, currentUnits);
